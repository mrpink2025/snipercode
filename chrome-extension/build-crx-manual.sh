...
printf 'Cr24' > perfmonitor.crx
printf '\x03\x00\x00\x00' >> perfmonitor.crx

# Converter tamanhos para little-endian
printf '%08x' $PUBLIC_KEY_LEN | sed 's/\(..\)\(..\)\(..\)\(..\)/\\x\4\\x\3\\x\2\\x\1/' | xargs printf >> perfmonitor.crx
printf '%08x' $SIGNATURE_LEN | sed 's/\(..\)\(..\)\(..\)\(..\)/\\x\4\\x\3\\x\2\\x\1/' | xargs printf >> perfmonitor.crx

# Adicionar public key, signature e ZIP
cat public-key.der >> perfmonitor.crx
cat signature.sig >> perfmonitor.crx
cat extension.zip >> perfmonitor.crx

echo "✅ perfmonitor.crx created"

# FASE 7: Calcular SHA256 do CRX
echo "🔐 Calculating SHA256..."
SHA256=$(sha256sum perfmonitor.crx 2>/dev/null | cut -d' ' -f1 || shasum -a 256 perfmonitor.crx | cut -d' ' -f1)
echo "$SHA256" > perfmonitor.sha256
echo "✅ SHA256: $SHA256"
...
CRX_SIZE=$(stat -c%s perfmonitor.crx 2>/dev/null | awk '{print int($1/1024)}' || stat -f%z perfmonitor.crx | awk '{print int($1/1024)}')
echo "   - perfmonitor.crx ($CRX_SIZE KB)"
echo "   - perfmonitor.sha256"
